...
import JinMoriRobot.modules.sql.welcome_sql as sql
from JinMoriRobot import (DEV_USERS, LOGGER, OWNER_ID, DRAGONS, DEMONS, TIGERS,
                          WOLVES, sw, dispatcher, JOIN_LOGGER)
from SaitamaRobot.modules.helper_funcs.chat_status import (
    is_user_ban_protected,
    user_admin,
)
from JinMoriRobot.modules.helper_funcs.misc import build_keyboard, revert_buttons
from JinMoriRobot.modules.helper_funcs.msg_types import get_welcome_type
        ...
        if should_welc:
            ...
            if new_mem.id == OWNER_ID:
                ...
                welcome_log = (f"{html.escape(chat.title)}\n"
                               f"#USER_JOINED\n"
                               f"Bot's Owner just joined the chat")
                continue

            # Welcome Devs
            elif new_mem.id in DEV_USERS:
                update.effective_message.reply_text(
                    "Whoa! A student of the God Of Highschool just joined!",
                    reply_to_message_id=reply,
                )
                continue
